...
// 성능 최적화: previewRevenue debounce
let _rvPreviewDebounce = null;
function debouncedPreviewRevenue() {
  clearTimeout(_rvPreviewDebounce);
  _rvPreviewDebounce = setTimeout(previewRevenue, 150);
}
...
function renderRevenue() {
  ...
  // 성능 최적화: assignment lookup map for worker pay
  const _assignMap = {};
  adminData.assignments.forEach(a => {
    if (a.month !== revenueMonth) return;
  ...
  const rows = activeCompanies.map(c => {
    ...
    const workerPay = fin?.worker_pay_total || (_assignMap[c.id] || []).reduce((s, a) => s + calcAssignmentFinalPay(a, finMap), 0);
  ...
  const html = `
    ...
    <div class="field">
      ...
      <input id="rvContract" type="number" value="${contractAmount}" placeholder="월 계약 금액"
             oninput="debouncedPreviewRevenue()">
    </div>
